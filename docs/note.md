...
3. 提示词注入（可选）：`rm_r1/dataset/mix_data/preprocess_data.py` 给每条样本 prepend system prompt。  
4. 训练入口：`rm_r1/scripts/RLVR/local/train_rm_r1_rlvr_dpsk_distilled_7b.sh` 启动 Ray + Hydra 配置覆盖。  
5. 训练主流程：`rm_r1/verl/trainer/main_ppo.py` 动态加载奖励函数，组装 Actor/Critic/Ref，启动 `RubricRMRayPPOTrainer.fit()`。  
6. 数据加载：`rm_r1/verl/utils/dataset/rl_dataset.py` 读取 JSONL，映射到 `context_messages + winner`，套 chat template 后 tokenization。  
7. 奖励计算：`rm_r1/verl/utils/reward_score/lm_as_judge.py` 只检查输出尾部是否包含正确 `<answer>[[A/B]]</answer>`。  
8. 推理验证：`demo/demo.py` 加载导出的 HF 模型，给定 A/B 回答并生成最终判定。
...
## 4. 训练与微调链路（GRPO）
### 4.1 训练启动（Shell 层）
`train_rm_r1_rlvr_dpsk_distilled_7b.sh` 做了三件事：
1. 配置基础超参（LR、batch、max prompt/response、GPU 利用率等）；  
2. 启停 Ray 集群；  
3. 调 `python -m rm_r1.verl.trainer.main_ppo` 并通过 Hydra 覆盖配置。
