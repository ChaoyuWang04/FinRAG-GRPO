...
# =================================================================
# -- 基础模型配置 --
MODEL_PATH=/workspace/models/dsr1-1.5b   # 指定要进行微调的基础模型（Policy Model）所在的路径。
...
# -- 显存与批大小 --
GPU_MEM_UTILIZATION=0.4               # vLLM 引擎的显存占用率。如果遇到 OOM (Out of Memory)，可以调低此值。
...
TRAIN_BS=8                           # [核心] Rollout 批大小。决定了每个 step 从数据集中取多少条 prompt 用于生成。
PPO_MINI_BS=4                         # [核心] PPO 训练批大小。在 Rollout 之后，用于模型参数更新的批次大小。

# -- 学习率预热 --
WARMUP_STYLE=constant                 # 学习率预热策略，'constant' 表示不使用预热。

# -- Token 长度控制 --
MAX_PROMPT_LENGTH=2048                # Prompt 的最大长度（tokens）。如果遇到 OOM，可以调低。
MAX_RESPONSE_LENGTH=2048              # 生成 Response 的最大长度（tokens）。如果遇到 OOM，可以调低。

# -- 分布式训练配置 --
TRAIN_PER_GPU=2                       # 每个 GPU 上进行 PPO 更新时的真实批大小。必须能被 PPO_MINI_BS 整除。
FORWARD_PER_GPU=2                     # 在计算 logprob 等前向传播任务时，每个 GPU 的批大小。必须能被 TRAIN_BS 整除。

# =================================================================
# === 日志与保存设置 ===
# =================================================================
PROJECT_NAME=RM-R1
EXPERIMENT_NAME=RM-R1-Dpsk-Distilled-1.5B_v1-LR${LR} # 实验名称，会用作日志和保存目录的一部分。
SAVE_NAME=RM-R1-Dpsk-Distilled-1.5B_v1-LR${LR}       # 保存 checkpoint 的目录名。
SAVE_META_DIR="/workspace/RM/outputs/2026-03-19"            # 所有实验结果、日志和模型的根目录。
...
# === 数据集设置 ===
# =================================================================
TRAIN_TASK="/workspace/RM/rm_r1/dataset/mix_data/train_with_sys.jsonl"  # 训练数据集的路径。
EVAL_TASK="/workspace/RM/rm_r1/dataset/mix_data/test_with_sys.jsonl"   # 验证数据集的路径。
