...
class RubricRMDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        # Support both HuggingFace hub and local files
        if self.files.startswith('shenwei/'):
            # Load from HuggingFace hub
            self.dataset = load_dataset(self.files)['train'].to_dict()
        else:
            # Load from local file (jsonl format)
            self.dataset = load_dataset('json', data_files=self.files)['train'].to_dict()
